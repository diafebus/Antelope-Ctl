...
def _link_state_path(profile):
    """Where to cache CLI-issued link state for this device (by vid/pid).
    Returns None (caller should just skip caching) if $HOME isn't usable --
    this is a convenience cache, never something to hard-fail a command over."""
    try:
        ...
        return os.path.join(d, f'link_state_{vid:04x}_{pid:04x}.json')
    except OSError:
        return None


def _load_link_state(profile):
    path = _link_state_path(profile)
...
def _is_pair_linked(profile, link_state, ch):
    ...
    this CLI has issued for that pair."""
    return bool(link_state.get(str(ch // 2)))


def _save_link_state(profile, pair, enabled):
    path = _link_state_path(profile)
    if not path:
        return
    pairs = _load_link_state(profile)
