...
- **AuraVerb has a live readback now** (cat `0x0a`). `antelope-ctl auraverb`
  reads the device (all 4 mixes) and does a verified read-modify-write;
  `protocol.parse_auraverb_record(profile, body)` decodes it. If/when the
  webUI grows an AuraVerb panel it can show real state, not a cache.
...
4. **Re-sweep emuMic pattern range** for models 1/12/16/18. ~~Confirm the
   ...
   model select audibly correct.) An AuraVerb panel is now feasible
   (live readback cat `0x0a`).
