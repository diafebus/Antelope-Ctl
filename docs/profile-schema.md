...
| `surround_global_command` | SET_SURROUND (`0xab`/`0xeb`) | complete-state template, verified 2.0 format flags, `runtime_operations`, bounded delay/level contract, readback cat/index | `build_surround_global_command(...)`; the WebUI performs a fresh read-modify-write of only global delay or level |
| — (no `frame.*` block) | SET_SURROUND_SPEAKER (`0x87`/`0xea`, per-speaker ×16) | documented byte-for-byte in `params.surround_speaker.field_map` | **no builder** — the WebUI/CLI decode category `0x1a` EQ readback only; candidate head and mode writes remain capture-gated |
...
`parse_mic_emulations`, `parse_afx_strip_order`,
`parse_surround_speaker_eq_record`, and `parse_surround_global_record`. These parsers only decode
